...
static int             show_all_matched_objects(FILE *, const char *, oid *,
                                         size_t *, int, int);

static void
usage(void)
{
    fprintf(stderr, "USAGE: translate [OPTIONS] OID [OID]...\n\n");
...
int
snmptranslate(int argc, char *argv[])
{
    int             arg;
    ...
    while ((arg = getopt(argc, argv, "Vhm:M:w:D:P:T:O:I:L:")) != EOF) {
	    ...
	    width = atoi(optarg);
	    if (width <= 0) {
    		printres("Invalid width specification: %s\n", optarg);
	    	return(1);
	    }
	    break;
#ifndef NETSNMP_DISABLE_MIB_LOADING
        case 'P':
            ...
            if (cp != NULL) {
                printres("Unknown parser option to -P: %c.\n", *cp);
                return(1);
            }
            break;
#endif /* NETSNMP_DISABLE_MIB_LOADING */
        case 'O':
            ...
            if (cp != NULL) {
                printres("Unknown OID option to -O: %c.\n", *cp);
                return(1);
            }
            break;
        case 'I':
            ...
            if (cp != NULL) {
                printres("Unknown OID option to -I: %c.\n", *cp);
                return(1);
            }
            break;
        case 'T':
            for (cp = optarg; *cp; cp++) {
                ...
                case 'a':
                    ...
                    break;
                default:
                    printres("Invalid -T<lostpad> character: %c\n",
                            *cp);
                    return(1);
                    break;
                }
    ...
    if (current_name == NULL) {
        switch (print) {
        default:
            printres("Syntax error. Try --help\n");
            return(1);
#ifndef NETSNMP_DISABLE_MIB_LOADING
        case 1:
    ...
    do {
        ...
        if (snmp_get_random_access()) {
...
#endif /* NETSNMP_DISABLE_MIB_LOADING */
                printres("Unknown object identifier: %s\n",
                        current_name);
                return(2);
#ifndef NETSNMP_DISABLE_MIB_LOADING
            }
            ...
            if (0 == show_all_matched_objects(stdout, current_name,
                                              name, &name_length,
                                              description, width)) {
                printres("Unable to find a matching object identifier for \"%s\"\n",
                        current_name);
                return(1);
            }
            return(0);
            ...
            if (!read_objid(current_name, name, &name_length)) {
                snmptools_snmp_perror(current_name);
                return(2);
            }
        }
        ...
        if (print == 1) {
            ...
            printres("%s", snmptools_print_objid(name, name_length));
            if (description) {
#ifndef NETSNMP_DISABLE_MIB_LOADING
                snmptools_print_description(name, name_length, width);
#endif /* NETSNMP_DISABLE_MIB_LOADING */
            }
        }
        current_name = argv[++optind];
