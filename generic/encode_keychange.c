 ...
 * Prototypes.
 */
static void            usage_to_file(FILE * ofp);
static void            usage_synopsis(FILE * ofp);
static int             get_user_passphrases(void);
static int             snmp_ttyecho(const int fd, const int echo);
static char           *snmp_getpassphrase(const char *prompt, int fvisible);

#ifdef WIN32
#define HAVE_GETPASS 1
static char           *getpass(const char *prompt);
static int             isatty(int);
static int             _cputs(const char *);
 ...
 */
int
encode_keychange(int argc, char *argv[])
{
