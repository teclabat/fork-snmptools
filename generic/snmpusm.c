...
snmpucm(int argc, char *argv[])
{
    ...
    if (ss == NULL) {
         ...
         */
        snmptools_snmp_sess_perror("snmpusm", &session);
        return(1);
    }
	...
	if (doprivkey) {
	  ...
	  if (rval != SNMPERR_SUCCESS) {
        ...
        if (status != SNMPERR_SUCCESS || dhresponse == NULL ||
            ...
            snmptools_snmp_sess_perror("snmpusm", ss);
    ...
    if (status == STAT_SUCCESS) {
        ...
        snmptools_snmp_sess_perror("snmpset", ss);
        exitval = 1;
    }
